...
class LevelForm extends Component {
    ...
    saveRoom = () => {
        ...
        axios.post(server + "api/rooms/levels/store", inputs, token)
        .then(response => {
            ...
            if( response.data.error ) {
                ...
                Alert.alert("Message", edit ? "Level has been saved!" : "New Level has been added.", 
                [{ text: "Okay", 
                    onPress: () => this.props.navigation.navigate("Levels", { saved, key, event } )
                }] )
            }
        })
        .catch(error => {
    ...
    render() {
        ...
        return (
            <Container>
                <Nav 
                title={inputs.id != 0 ? "Edit Level" : "New Level"}
                left={{ icon: "arrow-back", onPress: () => this.props.navigation.goBack() }}
                />
                <Content padder >
                    <Item regular style={[{ borderRadius: 5 }, style.formControll ]}>
                        <Input defaultValue={inputs.level} onChangeText={(txt) => this.handleChange("level", txt)} placeholder={"Enter Level Name"} />
                    </Item>
                    <Button style={{ elevation: 0 }} block onPress={() => this.saveRoom()}>
                        {
                            loading ? (<Spinner color={"#ffffff"}/>) : (<Text>Save Level</Text>)
                        }
                    </Button>
                </Content>
            </Container>
