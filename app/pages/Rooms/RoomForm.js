...
class RoomForm extends Component {
    ...
    saveRoom = () => {
        ...
        axios.post(server + "api/rooms/store", inputs, token)
        .then(response => {
            ...
            if( response.data.error ) {
                ...
                Alert.alert("Message", edit ? "Section has been saved!" : "New section has been added.", 
                [{ text: "Okay", 
                    onPress: () => this.props.navigation.navigate("Rooms", { saved, key, event } )
                }] )
            }
        })
        .catch(error => {
    ...
    render() {
        ...
        return (
            <Container>
                <Nav 
                title={inputs.id != 0 ? "Edit Section" : "New Section"}
                left={{ icon: "arrow-back", onPress: () => this.props.navigation.goBack() }}
                />
                <Content padder >
                    <Item regular style={[{ borderRadius: 5 }, style.formControll ]}>
                        <Input defaultValue={inputs.roomname} onChangeText={(txt) => this.handleChange("roomname", txt)} placeholder={"Enter Section Name"} />
                    </Item>
                    <Button style={{ elevation: 0 }} block onPress={() => this.saveRoom()}>
                        {
                            loading ? (<Spinner color={"#ffffff"}/>) : (<Text>Save Section</Text>)
                        }
                    </Button>
                </Content>
            </Container>
