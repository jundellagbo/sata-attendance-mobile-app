...
class TracksForm extends Component {
    ...
    saveRoom = () => {
        ...
        axios.post(server + "api/rooms/tracks/store", inputs, token)
        .then(response => {
            ...
            if( response.data.error ) {
                ...
                Alert.alert("Message", edit ? "Track has been saved!" : "New Track has been added.", 
                [{ text: "Okay", 
                    onPress: () => this.props.navigation.navigate("Tracks", { saved, key, event } )
                }] )
            }
        })
        .catch(error => {
    ...
    render() {
        ...
        return (
            <Container>
                <Nav 
                title={inputs.id != 0 ? "Edit Track" : "New Track"}
                left={{ icon: "arrow-back", onPress: () => this.props.navigation.goBack() }}
                />
                <Content padder >
                    <Item regular style={[{ borderRadius: 5 }, style.formControll ]}>
                        <Input defaultValue={inputs.tracks} onChangeText={(txt) => this.handleChange("tracks", txt)} placeholder={"Enter Track Name"} />
                    </Item>
                    <Button style={{ elevation: 0 }} block onPress={() => this.saveRoom()}>
                        {
                            loading ? (<Spinner color={"#ffffff"}/>) : (<Text>Save Track</Text>)
                        }
                    </Button>
                </Content>
            </Container>
