...
class Levels extends Component {
  ...
  constructor( props ) {
    super( props )
    this.state = {
        strand: props.navigation.state.params.strand,
      loading: true,
      error: false,
      rooms: [],
      copy: [],
  ...
  fetchData = async () => {
    this.setState({ loading: true })
      await axios.get(server + "api/rooms/levels?id=" + this.state.strand.id, this.state.token, {timeout: 1000})
      .then(response => {
        if( this.mounted ) {
          this.setState({ loading: false, rooms: response.data.levels, error: false, copy: response.data.levels })
        }
      })
      .catch(error => {
        if( this.mounted ) {
  ...
  remove = ( id, key ) => {
    axios.get(server + "api/rooms/removev2?option=level&id=" + id, this.state.token, timeout)
    .then(response => {
  ...
  executeOption = ( index, row, key )=> {
    switch(index)
    {
      case 0:
      // Redirect View1
      this.props.navigation.navigate("Rooms", { level: row })
      break;

      case 1:
      // Edit Form
      this.props.navigation.navigate("LevelForm", { inputs: row, key })
      break;
  ...
  search = txt => {
    ...
    let filteredRooms = rooms.filter(item => {
      if(item.level.toLowerCase().match(text)) {
        return item;
      }
    })
              ...
              <View style={{ flex: 1 }}>
                <View style={{ padding: 13 }}>
                  <Item regular style={{ padding: 2 }}>
                    <Input onChangeText={(txt) => this.search(txt)} placeholder={"Search level"} />
                ...
                </View>
                {
                  ...
                  data={rooms}
                  keyExtractor={item => 'list-item-level-' + item.id}
                  ...
                  />
                  ) : (<NoData text={"No level record."} />)
                }
                
                {
                    user.role == 1 && (
                        <View style={{ padding: 13 }}>
                            <Button style={{ elevation: 0 }} block color={"primary"} onPress={() => this.props.navigation.navigate("LevelForm", { inputs: this.mutate(), key: 0 }) }>
                                <Text>New Level</Text>
                            </Button>
                        </View>
                    )
...
const options = ["Go to this level", "Edit level", "Delete level", "Cancel"]
const option2 = ["Go to this level", "Cancel"]
...
const RoomsList = ( props ) => (
  ...
  >
    <Left>
      <Text>{props.dataSource.level}</Text>
    </Left>
    <Right>
