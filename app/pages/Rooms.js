...
class Rooms extends Component {
  ...
  fetchData = async () => {
    this.setState({ loading: true })
      await axios.get(server + "api/rooms?id=" + this.state.level.id, this.state.token, {timeout: 1000})
      .then(response => {
        if( this.mounted ) {
          this.setState({ loading: false, rooms: response.data.sections, error: false, copy: response.data.sections })
        }
      })
      .catch(error => {
        if( this.mounted ) {
  ...
  remove = ( id, key ) => {
    axios.get(server + "api/rooms/remove?id=" + id, this.state.token, timeout)
    .then(response => {
  ...
  executeOption = ( index, row, key )=> {
    switch(index)
    {
      case 0:
      // Redirect View1
      this.props.navigation.navigate("Students", { room: row })
      break;

      case 1:
      // Edit Form
      this.props.navigation.navigate("RoomForm", { inputs: row, key })
      break;
  ...
  search = txt => {
    ...
    let filteredRooms = rooms.filter(item => {
      if(item.roomname.toLowerCase().match(text)) {
        return item;
      }
    })
              ...
              <View style={{ flex: 1 }}>
                <View style={{ padding: 13 }}>
                  <Item regular style={{ padding: 2 }}>
                    <Input onChangeText={(txt) => this.search(txt)} placeholder={"Search section"} />
                ...
                </View>
                {
                  ...
                  data={rooms}
                  keyExtractor={item => 'list-item-section-' + item.id}
                  ...
                  />
                  ) : (<NoData text={"No section record."} />)
                }
                
                {
                    user.role == 1 && (
                        <View style={{ padding: 13 }}>
                            <Button style={{ elevation: 0 }} block color={"primary"} onPress={() => this.props.navigation.navigate("RoomForm", { inputs: this.mutate(), key: 0 }) }>
...
const options = ["Go to this section", "Edit section", "Delete section", "Cancel"]
const option2 = ["Go to this section", "Cancel"]
...
const RoomsList = ( props ) => (
  ...
  >
    <Left>
      <Text>{props.dataSource.roomname}</Text>
    </Left>
    <Right>
